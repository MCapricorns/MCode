...
> 对应 crate:`mycode-tools`
> 参考:grok-build `mycode-tools` 同构设计(Tool/ToolDyn/schemars/ToolStream)
...
终端投影属于 `UiPackService` / UiPack 的 typed UI 路径，不是 Core 到 ratatui 的直接后端。当前 `mycode-tui::render` 仅是迁移前实现状态；目标终端 substrate 在不支持 Unicode 时使用 ASCII 框线、将非 ASCII 内容替换为 `?`，并把截断标记改为 `.`，保证交给终端的整个缓冲区只含 ASCII。
...
在 builtin 或 Host adapter dispatch 前，Host 都只按已校验参数绑定一次 `PreparedSearch` / `PreparedFile`（能力绑定，不是授权）；不能复用旧参数、路径或句柄。动态 contribution 的 schema、名称空间、Manager、Pack ID/version/hash/generation 与资源边界均须在 adapter 建立时固定；通用 JSON 不可作为 Pack 语义通道。Ask 不属于工具 dispatch；产品 Ask 仅由 `com.mycode.ask` Pack 经 typed Service 实现。
...
MCP工具必须由`com.mycode.mcp` typed Service提供bounded namespaced contribution，再经Host adapter进入Registry；不得走旧direct transport。Ask只经`com.mycode.ask` typed Service运行，不是工具授权钩子。
