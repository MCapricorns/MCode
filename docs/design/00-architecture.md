...
MCode 是一个 GPUI 桌面应用（Windows 优先），没有 TUI/CLI/headless 前端。一切配置（Provider、搜索、MCP、User-Agent、主题）都在应用内的可视化设置页完成；密钥永不进入设置文件。

```text
mcode-desktop (GPUI, Windows)
  ├─ 纯 view-model reducer（无 GPU 依赖、可测）
  ├─ 渲染层：三栏布局（会话侧栏 / 对话流 / 上下文面板）
  └─ CoreBridge：一条专用线程 + current-thread tokio runtime
       ├─ mcode-session   持久会话 ledger（分支/恢复/回退、expected-head CAS）
       ├─ mcode-providers 三个 wire 协议适配器 + 可注入 SSE transport
       ├─ mcode-agent     双循环 agent（流式 → 工具 → 停止/steer）
       ├─ mcode-tools     内置工具 + ask_user/todo_write 交互
       ├─ mcode-web       bounded 搜索（URL/SSRF 防护）
       ├─ mcode-mcp       stdio + Streamable-HTTP 客户端
       └─ mcode-config    settings/secrets/todos/checkpoints/resources 严格文档
```
...
- **模型回合 = durable agent turn**：用户消息、工具结果、assistant 消息按事件落 ledger；`write`/`edit` 前自动 checkpoint（`~/.mcode/checkpoints/<session>/`），可一键回滚到会话前状态。
