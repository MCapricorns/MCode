...
pub const SECRETS_KIND: &str = "mcode-provider-secrets";
...
mod tests {
    ...
    #[test]
    fn missing_secrets_default_without_touching_disk() {
        let (parent, layout) = layout();
        let secrets = read_provider_secrets(&layout).expect("empty");
        assert_eq!(secrets, ProviderSecrets::new());
        assert!(secrets.key("p1").is_none());
        assert_eq!(std::fs::read_dir(parent.path()).expect("parent").count(), 0);
    }

    #[test]
    fn roundtrip_with_revision_cas_and_set_clear() {
        let (_parent, layout) = layout();
        let secrets = ProviderSecrets::new().with_key("zeta", Some("k-z"));
        let first = replace_provider_secrets(&layout, AuthorityRevision::ABSENT, &secrets)
            .expect("first publish");
        assert_eq!(first.get(), 1);

        let read = read_provider_secrets(&layout).expect("read");
        assert_eq!(read.key("zeta"), Some("k-z"));
        assert_eq!(read.provider_ids(), vec!["zeta"]);

        let stale = replace_provider_secrets(&layout, AuthorityRevision::ABSENT, &secrets);
        assert_eq!(
        ...
        let cleared = secrets
            .with_key("zeta", None)
            .with_key("alpha", Some("k-a"));
        let second = replace_provider_secrets(&layout, first, &cleared).expect("second publish");
        assert_eq!(second.get(), 2);
        let read = read_provider_secrets(&layout).expect("reread");
        assert_eq!(read.provider_ids(), vec!["alpha"], "keys stay sorted");
    }

    #[test]
    fn debug_never_exposes_key_material() {
        let secrets = ProviderSecrets::new().with_key("p1", Some("super-secret"));
    ...
    fn rejects_bad_ids_and_keys() {
        assert!(
            ProviderSecrets::new()
                .with_key("Bad Id", Some("k"))
                .validate()
                .is_err()
        );
        assert!(
            ProviderSecrets::new()
                .with_key("p1", Some(&"k".repeat(MAX_KEY_BYTES + 1)))
                .validate()
                .is_err()
        );
        assert!(
            ProviderSecrets::new()
                .with_key("p1", Some("line\nbreak"))
                .validate()
                .is_err()
        );
    }
