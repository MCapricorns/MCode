...
impl HttpChannel {
    ...
    async fn post_envelope(&self, envelope: Value) -> Result<Option<Value>, McpError> {
        let body = serde_json::to_vec(&envelope).map_err(|_| McpError::Protocol)?;
        ...
        let content_type = response
            ...
            .unwrap_or_default()
            .to_ascii_lowercase();
        let bytes = response.bytes().await.map_err(|_| McpError::Transport)?;
...
fn sse_reply(bytes: &[u8]) -> Result<Option<Value>, McpError> {
    let text = std::str::from_utf8(bytes).map_err(|_| McpError::Protocol)?;
    for line in text.lines() {
        ...
        let value: Value = serde_json::from_str(payload).map_err(|_| McpError::Protocol)?;
...
mod tests {
    ...
    fn id_mismatch_and_errors_fail_closed() {
        ...
        assert_eq!(finish_reply(Some(value), 7), Err(McpError::Protocol));
        let value = json!({"jsonrpc": "2.0", "id": 7, "error": {"code": -1}});
        assert_eq!(finish_reply(Some(value), 7), Err(McpError::ServerError));
