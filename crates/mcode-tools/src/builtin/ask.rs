...
/// Keeps the unused-import lint honest for Value in schema generation.
#[allow(unused)]
fn _value_used(v: &Value) -> &Value {
    v
}
...
mod tests {
    ...
    async fn ask_renders_answers_in_order() {
        ...
        let text = match &result.content[0] {
            mcode_core::message::ContentBlock::Text(text) => text.text.clone(),
