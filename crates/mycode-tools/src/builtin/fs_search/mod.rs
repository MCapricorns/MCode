...
// acquire_interrupt_signal is unix-only in `blocking`; both names are only
// referenced from unix-gated test code.
#[cfg(all(test, unix))]
pub(crate) use crate::builtin::blocking::{acquire_interrupt_signal, wait_for_worker_readable};
