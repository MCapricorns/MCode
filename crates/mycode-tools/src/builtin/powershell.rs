...
//!
//! MYCode uses `pwsh.exe` from `PATH` when it can be spawned. If it is absent,
//! this module installs a pinned Microsoft portable ZIP below the MYCode home
...
const INSTALL_RECORD_NAME: &str = "mycode-install.json";
...
pub(crate) async fn ensure_pwsh() -> Result<PathBuf, ToolError> {
    ...
    ensure_artifact(&mycode_home(), &artifact, source, true)
        .await
        .map_err(|err| {
...
fn mycode_home() -> PathBuf {
    mycode_home_from(
        std::env::var_os("MYCODE_HOME"),
        std::env::var_os("HOME"),
...
fn mycode_home_from(
    mycode_home: Option<OsString>,
    home: Option<OsString>,
    user_profile: Option<OsString>,
) -> PathBuf {
    if let Some(path) = mycode_home.filter(|value| !value.is_empty()) {
        ...
        ArtifactSource::Https(url) => {
            ...
            let request = async {
                let mut response = client
                    .get(url)
                    .header(USER_AGENT, "MYCode PowerShell bootstrap")
...
mod tests {
    ...
    #[test]
    fn home_resolution_uses_json_era_mycode_home_layout() {
        assert_eq!(
            mycode_home_from(Some(OsString::from("C:/managed")), None, None),
            PathBuf::from("C:/managed")
        );
        assert_eq!(
            mycode_home_from(None, Some(OsString::from("C:/home")), None),
            PathBuf::from("C:/home/.mycode")
        );
        assert_eq!(
            mycode_home_from(None, None, Some(OsString::from("C:/profile"))),
            PathBuf::from("C:/profile/.mycode")
        );
    }
