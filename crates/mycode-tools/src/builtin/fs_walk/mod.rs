...
use crate::builtin::fs_search::{
    ...
    PathOrderKey, ResolvedRoot, WalkLimiter, bind_current_limiter, child_name_in_parent,
    files_same_identity, is_hidden_skip, lossy_component, open_child_file, open_directory_nofollow,
...
pub(crate) fn walk_retained_tree(
    ...
    let _seams = bind_current_limiter(&root.limiter);
    if root.is_file() {
        return Ok(());
    }
