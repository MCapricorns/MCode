...
fn bootstrap_creates_only_the_exact_eager_directory_set() {
    ...
    assert!(!parent.path().join(".mycode").exists());
}
...
#[test]
fn home_derived_wrong_case_mycode_is_rejected_only_at_bootstrap() {
    let user_home = tempdir().expect("user home");
    fs::create_dir(user_home.path().join(".MYCODE")).expect("wrong case sibling");
    ...
    let error = ensure_home_layout(&layout).expect_err("wrong case .mycode");

    assert_eq!(error.kind(), ConfigErrorKind::InvalidHome);
    assert!(!error.to_string().contains(".MYCODE"));
...
fn missing_home_derived_parent_stays_lexical_without_creation() {
    ...
    assert_eq!(layout.root(), missing.join(".mycode"));
...
fn permission_denied_home_parent_remains_lexical_when_fixture_is_reliable() {
    ...
    assert_eq!(layout.root(), user_home.join(".mycode"));
...
fn unix_owned_links_are_rejected_but_prefix_links_are_followed() {
    ...
    let explicit_layout = HomeLayout::from_env(HomeEnv {
        mycode_home: Some(final_link.as_os_str().to_os_string()),
        home: None,
        user_profile: None,
    })
    ...
    assert!(outside.join(".mycode").is_dir());
    ...
    assert_eq!(derived.root(), prefix_link.join("real").join(".mycode"));
...
fn windows_user_home_junction_is_followed_outside_owned_boundary() {
    ...
    let explicit_layout = HomeLayout::from_env(HomeEnv {
        mycode_home: Some(linked_home.as_os_str().to_os_string()),
        home: None,
        user_profile: None,
    })
    .expect("explicit root remains lexical");
...
fn windows_intermediate_prefix_junction_user_home_stays_usable() {
    ...
    assert_eq!(layout.root(), prefix_link.join("real").join(".mycode"));
    assert!(real_base.join("real").join(".mycode").is_dir());
    assert!(!prefix_link.join(".mycode").exists());
}
