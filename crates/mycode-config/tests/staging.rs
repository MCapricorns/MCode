...
fn staging_is_lazy_until_begin_and_publishes_exact_journals() {
    ...
    let writing_bytes = format!(
        "{{\"formatVersion\":1,\"kind\":\"mycode-staging-transaction\",\"transactionId\":\"{}\",\"state\":\"writing\"}}\n",
        writing.id()
    );
    assert_eq!(
    ...
    let staged_bytes = format!(
        "{{\"formatVersion\":1,\"kind\":\"mycode-staging-transaction\",\"transactionId\":\"{id}\",\"state\":\"staged\"}}\n"
    );
    assert_eq!(
        fs::read(transaction.join("journal.json")).expect("journal"),
...
fn journal_compare_and_swap_rejects_every_noncanonical_writing_state_unchanged() {
    for (label, replacement) in [
        ...
        ("wrong id", br#"{"formatVersion":1,"kind":"mycode-staging-transaction","transactionId":"00000000000000000000000000000000","state":"writing"}
"#),
...
"#),
        ...
        let replacement = if label == "extra field" {
            format!(
                "{{\"formatVersion\":1,\"kind\":\"mycode-staging-transaction\",\"transactionId\":\"{}\",\"state\":\"writing\",\"extra\":true}}\n",
                writing.id()
            )
            .into_bytes()
        } else {
