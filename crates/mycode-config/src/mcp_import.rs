...
mod tests {
    ...
    #[test]
    fn normalize_strips_bearer_and_authorization() {
        assert_eq!(normalize_api_key("  sk-live-1  "), "sk-live-1");
        assert_eq!(normalize_api_key("Bearer sk-live-1"), "sk-live-1");
        assert_eq!(normalize_api_key("bearer   sk-live-1"), "sk-live-1");
        assert_eq!(
            normalize_api_key("Authorization: Bearer sk-live-1"),
            "sk-live-1"
        );
    }

    #[test]
    fn parses_claude_desktop_map_and_http_headers() {
        let imported = parse_mcp_import(
            r#"{
              "mcpServers": {
                "Brave Search": {
                  "command": "npx",
        ...
        let stdio = imported
            .iter()
            .find(|row| row.server.id == "brave-search")
            .expect("stdio");
        assert_eq!(stdio.server.transport, "stdio");
        assert_eq!(stdio.server.command.as_deref(), Some("npx"));
        assert_eq!(
            stdio.server.env.get("BRAVE_API_KEY").map(String::as_str),
            Some("from-env")
        );
        let http = imported
            .iter()
            .find(|row| row.server.id == "context7")
            .expect("http");
        assert_eq!(http.server.transport, "http");
        assert_eq!(http.api_key.as_deref(), Some("ctx-key"));
        assert_eq!(http.server.key_header.as_deref(), Some("bearer"));
    }

    #[test]
    fn rejects_http_without_https() {
        let error =
            parse_mcp_import(r#"{"mcpServers":{"local":{"url":"http://127.0.0.1:9/mcp"}}}"#)
                .expect_err("http");
