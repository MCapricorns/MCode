...
use mcode_core::{AssistantMessage, ContentBlock, Message, StopReason, ToolSpec, Usage};
        ...
        match event["type"].as_str().unwrap_or_default() {
            ...
            "response.failed" | "error" => {
                ...
                return vec![StreamEvent::Error(
                    mcode_provider_api::ProviderError::with_message(
                        mcode_provider_api::ProviderErrorKind::Rejected,
                        event["error"]["message"]
                            .as_str()
...
mod tests {
    use super::*;
    use mcode_core::{ToolResultMessage, UserMessage};
    ...
    fn body_converts_instructions_history_and_tools() {
        let request = Request::new()
            ...
            .with_message(Message::Assistant(AssistantMessage {
                blocks: vec![ContentBlock::ToolCall(mcode_core::ToolCall::new(
                    "call-1",
                    "read",
                    json!({"a": 1}),
                ))],
            ...
            .with_message(Message::ToolResult(ToolResultMessage {
                ...
                content: vec![ContentBlock::Text(mcode_core::TextBlock::new("ok"))],
                is_error: false,
                details: None,
            }))
            .with_tool(ToolSpec {
    ...
    fn failed_response_maps_to_rejected_error() {
        ...
        assert!(
            matches!(&events[0], StreamEvent::Error(e) if e.kind() == mcode_provider_api::ProviderErrorKind::Rejected)
        );
    }
}
