...
use mcode_config::AppSettings;
use mcode_core::ToolSpec;
use mcode_tools::{ToolCtx, ToolDyn, ToolError, ToolResult, ToolStream};
...
pub(crate) struct DynamicMcpTool {
    ...
    client: Arc<tokio::sync::Mutex<mcode_mcp::McpClient>>,
}
    ...
    async fn execute_dyn(
        ...
        let text = tokio::select! {
            ...
            result = call => result.map_err(|error| {
                ToolError::Execution(format!("MCP server '{}': {error}", self.server_id))
            })?,
        };
...
pub(crate) async fn connect_mcp_tools(
    ...
    let Ok(secrets) = mcode_config::read_provider_secrets(home) else {
        return tools;
    };
    for server in settings.mcp_servers.iter().filter(|server| server.enabled) {
        ...
        let channel: Arc<dyn mcode_mcp::JsonRpcChannel> = match server.transport.as_str() {
            "stdio" => {
                ...
                let Ok(channel) =
                    mcode_mcp::StdioChannel::spawn(command, &server.args, MCP_CONNECT_TIMEOUT)
                        .await
                else {
                    continue;
                };
            ...
            "http" => {
                ...
                let Ok(channel) = mcode_mcp::HttpChannel::new(
                    endpoint,
...
mod tests {
    ...
    #[async_trait]
    impl mcode_mcp::JsonRpcChannel for ScriptedChannel {
    ...
    async fn spec_carries_the_server_schema_and_calls_round_trip() {
        ...
        let ctx = ToolCtx::new(".");
        let mut stream = mcode_tools::stream::ToolStream::channel().0;
        let result = tool
            .execute_dyn(json!({"query": "rust"}), &ctx, &mut stream)
            .await
            .expect("call");
        match &result.content[0] {
            mcode_core::ContentBlock::Text(text) => assert_eq!(text.text, "found 3 pages"),
