...
/// Bundle kind marker.
pub const EXPORT_KIND: &str = "mcode-export";
...
pub fn import_from_file(home: &HomeLayout, path: &Path) -> Result<ImportSummary, String> {
    ...
    if bundle.format_version != EXPORT_FORMAT_VERSION || bundle.kind != EXPORT_KIND {
        return Err("not an MCode export bundle".to_owned());
    }
    bundle
        .settings
        .validate()
    ...
    for (session_id, body) in &bundle.todos {
        if !plain_name(session_id) || body.len() > mcode_config::MAX_SETTINGS_BYTES {
            continue;
        }
        let target = home
            .root()
...
fn read_current_revision(home: &HomeLayout) -> Result<AuthorityRevision, String> {
    let path = home.root().join(mcode_config::SETTINGS_PATH);
    if !path.exists() {
