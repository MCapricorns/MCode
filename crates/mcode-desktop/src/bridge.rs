...
mod tests {
    ...
    /// invalid request. Run with `cargo test -p mcode-desktop -- --ignored`.
    #[tokio::test]
    #[ignore = "calls the live provider configured under ~/.mcode"]
    async fn live_provider_round_trips_a_tool_call() {
        ...
        let call = assistant
            ...
            .find_map(|block| match block {
                mcode_core::ContentBlock::ToolCall(call) => Some(call.clone()),
                _ => None,
            })
        ...
        let follow_up = Request::new()
            ...
            .with_message(Message::ToolResult(mcode_core::ToolResultMessage {
                tool_call_id: call.id.clone(),
                content: vec![mcode_core::ContentBlock::Text(mcode_core::TextBlock::new(
                    "wrote hello-tool-e2e.txt",
                ))],
                is_error: false,
                details: None,
            }))
